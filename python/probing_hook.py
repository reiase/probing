...
try:
    ...
    if probe_value.lower() in ['1', 'followed']:
        print(f"Activating probing in 'followed' mode (current process only)", file=sys.stderr)
        import probing
        # Environment variable is intentionally not preserved
        
    elif probe_value.lower() in ['2', 'nested']:
        print(f"Activating probing in 'nested' mode (all child processes)", file=sys.stderr)
        import probing
        ...
        try:
            ...
            if re.search(pattern, current_script):
                print(f"Activating probing for script matching '{pattern}'", file=sys.stderr)
                import probing
        ...
        if probe_value == current_script:
            print(f"Activating probing for '{current_script}' (current process only)", file=sys.stderr)
            import probing

...
print(internal_directories)


def probing(func=None, watch=[], depth=1):
    if func is not None:
...
class ProbingTracer:
    ...
    def trace(self, frame: FrameType, event: AnyStr, arg: Any):
        ...
        # print(f"Event: {event}, Frame: {frame}, Arg: {arg}, name: {frame.f_code.co_name}")
...
def ProbingTensor(*args, **kwargs):
    ...
    return _ProbingTensor(*args, **kwargs)
...
if "probing" not in sys.modules:
    sys.modules["probing"] = ModuleType("probing")
sys.modules["probing"].probing = probing

if "probing.trace" not in sys.modules:
    sys.modules["probing.trace"] = ModuleType("probing.trace")
sys.modules["probing.trace"].list_traceable_functions = (
    TraceModule.list_traceable_functions
)

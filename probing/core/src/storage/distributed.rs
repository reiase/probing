...
impl DistributedStoreCoordinator {
    ...
    async fn allocate_storage_addresses<T: PersistentEntity>(
        ...
        // 尝试分配到本地，如果失败则使用标准分配
        let addresses = if let Ok(local_addresses) =
            self.try_allocate_locally(entity.id().as_str(), &allocator)
        {
            local_addresses
        } else {
            allocator
                .allocate_addresses(entity.id().as_str().to_string())
        ...
        for (i, address) in addresses.iter().enumerate() {
            let is_primary = i == 0;
            let is_local = address.is_local(&self.worker_id);

            locations.push(StorageLocation::new(address.clone(), is_primary, is_local));
        }
    ...
    fn try_allocate_locally(
        ...
        if let Some(workers) = topology.workers_per_node.get(&self.node_id) {
            if workers.contains(&self.worker_id) {
                // 本地可用，创建本地地址
                let local_address = Address::new(self.worker_id.clone(), object_id.to_string());
...
impl DistributedEntityStore {
    ...
    /// 获取实体的分布式元数据
    async fn get_metadata<T: PersistentEntity>(
        &self,
        id: &T::Id,
    ) -> Result<Option<DistributedMetadata>> {
        let metadata_key = format!("__metadata__:{}::{}", T::entity_type(), id.as_str());

        // 直接使用 metadata_key 作为 metadata entity 的 id
        match self
            .metadata_store
            .get::<DistributedMetadata>(&metadata_key.into())
            .await
        {
            Ok(data) => Ok(data),
            Err(_) => Ok(None),
...
impl EntityStore for DistributedEntityStore {
    async fn save<T: PersistentEntity>(&self, entity: &T) -> Result<()> {
        ...
        let write_locations = self
            .coordinator
            .select_write_locations(&locations, &metadata.consistency_level);

        // 4. 并发写入到选定位置
        let write_results = self
            .coordinator
            .write_to_locations(entity, &write_locations)
            .await?;
    ...
    async fn get<T: PersistentEntity>(&self, id: &T::Id) -> Result<Option<T>> {
        ...
        for location in metadata.local_locations() {
            if self.coordinator.check_location_health(location).await {
                if let Ok(Some(entity)) =
                    self.coordinator.read_from_location::<T>(id, location).await
                {
        ...
        if let Some(primary) = metadata.primary_location() {
            if self.coordinator.check_location_health(primary).await {
                if let Ok(Some(entity)) =
                    self.coordinator.read_from_location::<T>(id, primary).await
                {
                    return Ok(Some(entity));
                }
            }
    ...
    async fn delete<T: PersistentEntity>(&self, id: &T::Id) -> Result<()> {
        ...
        self.metadata_store
            .delete::<DistributedMetadata>(&metadata_entity_id)
            .await?;
    ...
    async fn find_by_index<T: PersistentEntity>(
        ...
        self.coordinator
            .local_store
            .find_by_index::<T>(index_name, index_value)
            .await
    }
...
mod tests {
    ...
    async fn test_storage_address_allocation() {
        ...
        let locations = coordinator
            .allocate_storage_addresses(&test_entity)
            .await
            .unwrap();
    ...
    async fn test_consistency_level_selection() {
        ...
        let locations = vec![
            StorageLocation::new(
                Address::new("worker1", "obj1".to_string()),
                true, // is_primary
                true, // is_local
            ),
            StorageLocation::new(
                Address::new("worker3", "obj1".to_string()),
                false, // is_primary
                false, // is_local
            ),
            StorageLocation::new(
                Address::new("worker4", "obj1".to_string()),
                false, // is_primary
                false, // is_local
            ),
        ];
        ...
        // 测试 Primary 级别
        let primary_writes =
            coordinator.select_write_locations(&locations, &ConsistencyLevel::Primary);
        assert_eq!(primary_writes.len(), 1);
        assert!(primary_writes[0].is_primary);

        // 测试 Quorum 级别
        let quorum_writes =
            coordinator.select_write_locations(&locations, &ConsistencyLevel::Quorum);
    ...
    async fn test_distributed_entity_store_local_operations() {
        ...
        // 验证地址分配
        let locations = distributed_store
            .coordinator
            .allocate_storage_addresses(&test_entity)
            .await
            .unwrap();
        ...
        // 验证写入位置选择
        let write_locations = distributed_store
            .coordinator
            .select_write_locations(&locations, &ConsistencyLevel::Primary);
        ...
        distributed_store
            ...
            .await
            .unwrap();
        let direct_result = distributed_store
            .coordinator
            .local_store
            .get::<TestEntity>(&test_entity.id)
            .await
            .unwrap();
        ...
        // 检查实体是否在本地存储中
        let local_check = distributed_store
            .coordinator
            .local_store
            .get::<TestEntity>(&test_entity.id)
            .await
        ...
        // 测试获取
        let retrieved = distributed_store
            .get::<TestEntity>(&test_entity.id)
            .await
            .unwrap();
        ...
        // 测试删除
        distributed_store
            .delete::<TestEntity>(&test_entity.id)
            .await
            .unwrap();
        let deleted = distributed_store
            .get::<TestEntity>(&test_entity.id)
            .await
            .unwrap();
    ...
    async fn test_remote_client_integration() {
        ...
        let remote_client =
            Arc::new(MemoryRemoteClient::new(remote_store)) as Arc<dyn RemoteStoreClient>;
        coordinator
            .add_remote_client("node2::worker3".to_string(), remote_client)
            .await;

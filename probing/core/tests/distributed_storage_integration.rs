...
use probing_core::storage::{
    DistributedEntityStore, DistributedStoreCoordinator, EntityStore,
    MemoryStore, TopologyView, MemoryRemoteClient
};
...
async fn create_test_cluster() -> Result<Vec<Arc<DistributedEntityStore>>> {
    ...
    for (node_id, workers) in &topology.workers_per_node {
        ...
        for (remote_node, _) in &topology.workers_per_node {
            if remote_node != node_id {
                ...
                coordinator.add_remote_client(remote_node.clone(), remote_client).await;
            }
        }
        
        let metadata_store = Arc::new(MemoryStore::new());
        let distributed_store = Arc::new(DistributedEntityStore::new(coordinator, metadata_store));
...
async fn test_multi_node_storage_and_retrieval() -> Result<()> {
    ...
    stores[0].save(&entity).await?;
    
    // Try to retrieve from different nodes
    for (i, store) in stores.iter().enumerate() {
        let retrieved = store.get::<TestEntity>(&entity.id).await?;
        assert!(retrieved.is_some(), "Node {} should be able to retrieve entity", i);
        assert_eq!(retrieved.unwrap(), entity, "Retrieved entity should match original");
    }
...
async fn test_consistency_levels() -> Result<()> {
    ...
    stores[0].save(&primary_entity).await?;
    ...
    stores[1].save(&quorum_entity).await?;
...
async fn test_delete_operations() -> Result<()> {
    ...
    // Delete entity
    stores[0].delete::<TestEntity>(&entity.id).await?;

...
pub static AUTH_USERNAME: Lazy<String> = Lazy::new(|| {
    env::var(AUTH_USERNAME_ENV).unwrap_or_else(|_| "admin".to_string())
});

pub static AUTH_REALM: Lazy<String> = Lazy::new(|| {
    env::var(AUTH_REALM_ENV).unwrap_or_else(|_| "Probe Server".to_string())
});
